...
def hash_password(password, salt=None, iterations=1):
    """
    Hashes a password using SHA-256 with optional salt and custom loop iterations.
    """
    if salt is None:
    ...
    current_hash = hashlib.sha256(salt_bytes + pwd_bytes).digest()
...
def verify_password(password, hashed_value):
    """
    Verifies a password against an iterated SHA-256 hash.
    """
    try:
...
def benchmark_hash(password, iterations=1):
    """
    Benchmarks SHA-256 hashing execution speed.
    Returns:
